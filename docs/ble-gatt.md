...
{"op":"rename_preset","setlist":0,"index":17,"name":"Essex Twin"}
{"op":"export_preset","setlist":0,"index":17}
{"op":"import_preset","setlist":0,"index":3,"hlx":{"data":{"meta":{"name":"Probe"},"tone":{}}}}
...
`select_snapshot` takes `"index":0`–`7` (opcode 88). `move_block` takes `"from"` and `"to"` USB slots 0–39 on one DSP (opcode 43, keys 75/76). Input, output, split, and merge are refused. `set_model` changes what a slot is (opcode 40, after a type-78 select). Pass `"model"` (Helix.sym number) or `"model_id"` (catalog id). Optional `"stereo":true|false` selects the stereo or mono firmware symbol when both exist for that id. Amp+Cab uses `"pair":true` or `"paired"` / `"paired_id"`. `clear_block` empties a slot (opcode 28). Input, output, split, and merge are refused. The GUI opens this from the inspector title or from an empty cell (category, then Mono/Stereo/Legacy when present, then models). `get_state` blocks include `"stereo"` when the catalog has both widths. Device error **-306** is DSP budget. The GUI error banner clears after five seconds. `save_preset` writes the edit buffer to a setlist slot (opcode 71, keys 107/108/109). Omit `setlist`, `index`, or `name` to use opcode 23 identity. `rename_preset` takes `"setlist"`, `"index"`, and `"name"` (opcode 6, same keys, 1–16 characters) and changes the stored slot label without reloading the tone. `export_preset` reads a stored slot without loading it (opcode 4) and returns `{filename, name, hlx}`. Unsaved edit-buffer changes are not in the file; Save first. Needs the HX Edit catalog. `import_preset` rebuilds the `.hlx` into a device-written template (the dest slot, or the loaded preset if that slot is empty) and writes it with opcode 8 (`write_preset_at`), the same as HX Edit. It does not load the dest unless that slot is already the playing preset. Switches and menus keep the MessagePack tags the Floor stores; extra keys the catalog does not name are ignored. The file's snapshots, `controller` assignments, and Command Centre / `footswitch` entries replace whatever the dest slot held. `hlx` must be a JSON object with `data.tone`, at most 2 MB. GATT framing is 16-bit, so large `.hlx` transfers fail on Bluetooth; use Wi-Fi. `events` returns `{dirty, setlist, index}` from drained notifications.
